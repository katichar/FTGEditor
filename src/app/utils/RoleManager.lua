...
rolemanager.allRole={}
function rolemanager.buildRole1(RID,PLayer)
	local i_info = clone(g_RoleConfig[RID])
	i_info.hp=99999
    i_info.mp=99999
    i_info.def=99999
    i_info.atk=1
	local role = RoleObject.new(RID)
	role:retain()
	role:buildAnimal(i_info)
	role.MaxHP = 10000
	-- role:setSkillInfo()
    role:init()
    role:setPosition(-200, 0)
    role:addTo(PLayer)
	role.collider:enable()
	return role
end
function rolemanager.buildNPC1(RID,PLayer)
	local i_info = clone(g_NPC[RID])
	i_info.hp=99999
    i_info.mp=99999
    i_info.def=99999
    i_info.atk=1
	local role = NPCObject.new(RID)
	role:retain()
	role:buildAnimal(i_info)
	role.MaxHP = 10000
	-- role:setSkillInfo()
...
function rolemanager.reAddPlayer(i_X,i_Y)
	...
	if rolemanager.player then
		...
		roleinfo.offSpeed=rolemanager.player.speed
